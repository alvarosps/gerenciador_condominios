...
**Status**: **S34–S40 concluídas** (Fase 1 + Fase 2 completa: BE + frontend data-layer + UI). Próxima: **S41** (Fase 3 BE — InstallmentPlan/Installment + Employee + convert_deferred + estende geração). Gate por fase mantido (≥90% standalone em `finances` no BE; vitest/tsc/eslint 0/0 no FE).
