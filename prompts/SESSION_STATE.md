...
**Status**: **S34–S45 concluídas** (Fase 1 + Fase 2 completa + Fase 3 BE+FE + **Fase 4 BE completa**: modelos + `CondoBalanceService`/`CondoMonthCloseService`/`ReserveService` + guarda de saldo no `pay(funded_from=reserve)` + API + dashboard). Próxima: **S46** (Frontend Fase 4 — KPIs Caixa/Reserva/Resultado/Atrasados/Saldo total + Reserva + Receita avulsa + Fechamento). Gate por fase mantido (≥90% standalone em `finances` no BE; vitest/tsc/eslint 0/0 no FE).
