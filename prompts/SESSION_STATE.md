...
**Status**: **S34–S41 concluídas** (Fase 1 + Fase 2 completa + Fase 3 BE: parcelas/folha). Próxima: **S42** (API da Fase 3 — serializers/viewsets/URLs de `installment-plans`/`installments`/`employees` + `convert_deferred`). Gate por fase mantido (≥90% standalone em `finances` no BE; vitest/tsc/eslint 0/0 no FE).
