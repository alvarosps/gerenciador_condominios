---
paths:
  - "frontend/**"
---

# Frontend Rules

## State Management
- Server state: TanStack Query v5 — never use useState/useEffect for API data
- Client state: Zustand (auth only) — don't expand Zustand for server state
- Form state: React Hook Form + Zod — never use Ant Design Form components
- Simulation state: ephemeral (useState only) — scenarios are not persisted

## Component Patterns
- CRUD pages: always use `useCrudPage` hook from `lib/hooks/use-crud-page.ts`
- Tables: use `DataTable` from `components/tables/`
- Dialogs: use `DeleteConfirmDialog` for delete confirmations
- Layout: all dashboard pages wrap in `MainLayout`
- Charts: use Recharts (ComposedChart for bar+line, PieChart for breakdowns)
- Dashboard widgets: each widget is an independent component with its own hook

## Ant Design + Tailwind
- Use Ant Design for data-heavy components (Table, Select, DatePicker, Form layout)
- Use Tailwind for spacing, colors, custom layouts
- Use Shadcn/ui for dialogs, sheets, tooltips (Radix-based)
- Don't mix: pick one system per component type

## API Communication
- All API calls go through `lib/api/client.ts` Axios instance
- Create TanStack Query hooks in `lib/api/hooks/` — never call axios directly from components
- Error handling: use `getErrorMessage()` and `handleError()` from `lib/utils/error-handler.ts`
- Mutations must invalidate relevant query caches on success: `void queryClient.invalidateQueries(...)`

## TypeScript Strict
- Use `import type` for type-only imports
- Use `??` instead of `||` for nullable values
- Add null guards for array index access (noUncheckedIndexedAccess is enabled)
- Never use `as` type assertions or `!` non-null assertions — use proper type narrowing

## Financial Module Patterns
- Expense form adapts fields based on expense_type — use conditional rendering with watch()
- Cascading filters: person → credit card (card options filter by selected person)
- Installments: view in Sheet/Drawer component, mark_paid via PATCH action
- Currency formatting: use formatCurrency() from lib/utils/formatters.ts (R$ 1.500,00)
- Date formatting: DD/MM/YYYY via date-fns format with pt-BR locale
- Dashboard widgets are independent — each fetches its own data

## Tenant Wizard
- 6-step wizard in `tenants/_components/wizard/`
- Each step is a separate component with its own validation
- Don't break this pattern — new tenant fields go in the appropriate step
