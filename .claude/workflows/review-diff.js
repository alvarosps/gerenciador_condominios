...
const LENSES = [
  ...
  { key: 'data-safety', prompt: 'Soft delete & data safety (.claude/rules/database.md): default querysets exclude soft-deleted — must use .with_deleted() when deleted rows are needed; no hard_delete without justification; destructive migrations/ops require a pg_dump backup first (scripts/backup_db.py); never flush/reset on real data; new business models declare AuditMixin/SoftDeleteMixin unless intentionally exempt (IPCAIndex, MonthSnapshot, FinancialSettings, Notification, DeviceToken, WhatsAppVerification, OAuthExchangeCode, ExpenseMonthSkip).' },
  { key: 'security', prompt: 'Security (.claude/rules/security.md): JWT auth required on all endpoints except /api/token*, /api/auth/google; the financial module (legacy core + finances/ app) is admin-only via IsAdminUser (non-staff get 403 on read+write); IsAdminUser imported from core.permissions (not rest_framework.permissions); Tenant/Lease querysets scoped to the requesting tenant when non-staff; no raw SQL or string-formatted queries (Django ORM / parameterized only); no secrets/tokens/credentials in code or logs; CPF/CNPJ validated via core/validators on relevant fields.' },
