...
const TYPE_BARS = [
  { key: 'card_purchases' as const, label: 'Cartão', color: chartColor(0) },
  { key: 'utility_bills' as const, label: 'Contas', color: chartColor(1) },
  { key: 'loans' as const, label: 'Empréstimos', color: chartColor(2) },
  { key: 'one_time_expenses' as const, label: 'Únicos', color: chartColor(3) },
  { key: 'fixed_expenses' as const, label: 'Fixos', color: chartColor(4) },
] as const;
...
function CustomTooltip({
  ...
  return (
    <div className="bg-card border rounded-lg shadow-lg p-3">
      <p className="font-medium" style={{ color: first.payload.color }}>
        {label}
      </p>
      <p className="text-sm">
