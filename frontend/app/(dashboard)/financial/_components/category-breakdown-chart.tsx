...
export function CategoryBreakdownChart() {
  ...
  const { data, isLoading, error } = useCategoryBreakdown(currentYear, currentMonth);

  const monthName = MONTH_NAMES[currentMonth - 1] ?? '';
  const title = `Despesas por Categoria — ${monthName}/${currentYear}`;
  ...
  if (error || !data) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-center text-muted-foreground py-8">Erro ao carregar categorias</p>
        </CardContent>
      </Card>
    );
  }
