...
> - **P6.1 FRONTEND EXECUTADO** (2b96e14): os **24** testes de componente de `finances/` agora usam hooks reais contra MSW — **zero** `vi.mock('@/lib/api/hooks')`/`sonner`/`auth-store` por arquivo (sonner = mock global em setup.ts; store real via `setState`). Shape bruto corrigido (`tests/mocks/data/finances.ts` tipado `z.input<schema>` → dinheiro string, sem `*_id` no read; props parseiam o mock cru, ex. `reserveSchema.parse`). Mutations afirmadas via **spy do corpo da request no MSW** (`server.use`). Flags virados: removido `dangerouslyIgnoreUnhandledErrors` + `onUnhandledRequest:'error'`. Infra: `renderWithProviders` retorna `{queryClient}` + `waitForQueriesToSettle` (drena GETs em voo antes do teardown). 2 leaks latentes pré-existentes expostos pelos flags e corrigidos (`financial/daily` admin-gating; `error-handler.test.ts` outbound real → `HttpResponse.error()`). Padrão: 3 arquétipos provados por mim + 15 arquivos via 4 subagentes paralelos por subtree; infra + flip + verificação agregada minhas. Gate verde: eslint/tsc limpos, **945 testes / 0 unhandled errors**.
