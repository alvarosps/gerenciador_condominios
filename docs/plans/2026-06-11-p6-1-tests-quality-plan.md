...
> **Prioridade:** FASE P6 · **Branch:** `perf/p5-p6` (combinado P5+P6, 1 PR) · **Depende de:** nenhum bloqueante; consolida gaps transversais de qualidade de teste.
>
> **Revisão 2026-06-13 (verificação pós-P5):** anchors re-verificados.
> - **Escopo frontend = 18 arquivos** de `app/(dashboard)/finances/**/__tests__/*.tsx` que mockam `@/lib/api/hooks/*` (NÃO 25/26 — esse é o total do repo incl. 8 de legado/admin que a constraint adia). `data/finances` é um **arquivo único** (`finances.ts`, 498 linhas), NÃO um diretório. A migração é um **esforço grande em 3 tiers** (não um find-replace): Tier 1 query-only (bill-line-items-field; o GET de bills-page já é MSW via P5.2); Tier 2 mutation+form (9 modais/dialogs, vários com Radix Select sob `pointerEventsCheck:0`); Tier 3 list-pages que mockam o read-query + stubs de modal filho (installment-plans-page, employees-page, finance-categories-page, income-entries/page, month-close/page, reserve/page, bills-page-import). Ordem: **corrigir o shape bruto de `data/finances.ts` PRIMEIRO** (decimais number→string, remover `*_id` write-only dos reads) → Tier 1 → Tier 2 (subtree bills/ como referência) → Tier 3 → só então virar `onUnhandledRequest:'error'` (senão testes não-migrados quebram em massa).
> - **MSW shape divergência CONFIRMADA:** `createMockBill/BillLineItem/Installment/InstallmentPlan/Employee/Reserve/IncomeEntry/Payment/BillingAccount` emitem decimais como **número** (ex.: `amount: 350`) e incluem `*_id` write-only em reads; o real DRF emite **string** ("350.00") e nunca `*_id` no read. (O mesmo arquivo já acerta strings nos geradores de dashboard/projection — tornar os CRUD consistentes.)
> - **Item 10 (late_fee cross-month/fev) — JÁ SATISFEITO** no master: `test_fee_calculator.py::test_late_fee_cross_month_overdue` (68-78) e `::test_due_day_31_overdue_in_february` (80-89). **NÃO duplicar.** (método real em fee_calculator.py:50-110.)
> - **Twilio (seção 6):** são **8** patches de `core.viewsets.auth_views.send_verification_code` (não 7), em test_tenant_auth_api.py:**86,98,117,136,162,229,258,269**; o docstring do módulo (8-9) rotula erradamente `send_verification_code` como "external boundary" → reescrever. **`TWILIO_ACCOUNT_SID` NÃO está setado no env de teste** (pytest.ini env 92-98 não tem TWILIO_*) → mockar `Client` sozinho NÃO basta: `send_whatsapp_message` levanta `RuntimeError` em settings.py guard antes do Client. **Obrigatório** `@override_settings(TWILIO_ACCOUNT_SID=..., TWILIO_AUTH_TOKEN=..., TWILIO_TEMPLATE_VERIFICATION=...)`. Boundary real: `core.services.whatsapp_service.Client` (import :6), guard :59-61, Client() :63, json.dumps content_variables :68. `_generate_valid_cpf` deve **reusar** `CPFValidator.calculate_checksum_digit` (brazilian.py:63-77), não re-implementar.
> - **Anchors corrigidos:** pytest.ini filterwarnings 79-83 (exato; addopts real `-n 2 --dist=loadscope`, `--cov-fail-under=90`); factories TEST_CPFS 12-28 / _cpf_cycle :30 / _next_cpf :35-36 / make_tenant usa em :79; conftest invoice write_bytes :44 (exato); test_finance_permissions WRITE_ENDPOINTS 8-19 (exato), rotas reais close @crud_views.py:784-786, reopen :788-790, PaymentViewSet :161 destroy→unpay :200-213; vitest dangerouslyIgnoreUnhandledErrors :18 (exato), setup.ts onUnhandledRequest :7; bills-page.test.tsx vi.mock :12-15.
