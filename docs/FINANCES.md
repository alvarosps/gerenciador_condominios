...
**Actions:** `bills/{id}/{pay,suspend,defer,cancel,reactivate}/`, `bills/{bulk_pay,generate_month,create_with_lines,parse_invoice,create_purchase}/`, `bills/{id}/{update_with_lines,apply_invoice,delete_purchase,reassign_payer}/`, `billing-accounts/{id}/{statement,consolidate_debt}/`, `condo-month-closes/{close,reopen}/`, `finance-dashboard/{overview,monthly_balance,iptu_alerts,overdue,combined_calendar,by_category,by_owner,month_board}`, `finance-cash-flow/{projection,simulate}`, `third-party/{people,statement}`.
