...
class TestTenantCreationFlow(BaseE2ETest):
    ...
    def test_create_tenant_with_dependents_flow(self):
        ...
        update_data = {**tenant_data, "profession": "Surgeon"}
    ...
    def test_tenant_with_furniture_flow(self):
        ...
        tenant = self.create_tenant(name="Pedro Santos", furniture_ids=[furniture1["id"], furniture2["id"]])
...
class TestLeaseCreationFlow(BaseE2ETest):
    ...
    def test_complete_lease_creation_workflow(self):
        ...
        self.assert_response_success(response, 200)
        _apt_updated = response.json()  # noqa: F841
    ...
    def test_lease_validation_flow(self):
        ...
        lease1 = self.create_lease(apartment["id"], tenant["id"], [tenant["id"]])
        ...
        lease2 = self.create_lease(apartment2["id"], tenant2["id"], [tenant2["id"]])
...
class TestContractGenerationFlow(BaseE2ETest):
    ...
    def test_contract_regeneration_flow(self):
        ...
        _pdf_path1 = response1.json()["pdf_path"]  # noqa: F841
...
class TestLateFeesAndDueDateFlow(BaseE2ETest):
    ...
    def test_late_fee_calculation_flow(self):
        ...
        lease = self.create_lease(apartment["id"], tenant["id"], [tenant["id"]], rental_value="6000.00")
    ...
    def test_due_date_change_flow(self):
        ...
        lease = self.create_lease(apartment["id"], tenant["id"], [tenant["id"]], due_day=10, rental_value="2000.00")

        # Step 2: Change due date
        response = self.client.post(f'/api/leases/{lease["id"]}/change_due_date/', {"new_due_day": 5}, format="json")
        self.assert_response_success(response, 200)
        result = response.json()
...
class TestLeaseLifecycleFlow(BaseE2ETest):
    ...
    def test_complete_lease_lifecycle(self):
        ...
        # Step 2: Create lease
        lease = self.create_lease(apartment["id"], tenant1["id"], [tenant1["id"], tenant2["id"]])
        lease_id = lease["id"]
        ...
        response = self.client.get(f"/api/leases/{lease_id}/calculate_late_fee/", {"days_late": 3})
        self.assert_response_success(response, 200)

        # Step 5: Change due date
        response = self.client.post(f"/api/leases/{lease_id}/change_due_date/", {"new_due_day": 15}, format="json")
        self.assert_response_success(response, 200)
        ...
        assert any(lease["id"] == lease_id for lease in leases)

        # Filter by building
        response = self.client.get("/api/leases/", {"apartment__building": building["id"]})
        self.assert_response_success(response, 200)
