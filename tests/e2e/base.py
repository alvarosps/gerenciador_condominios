...
class BaseE2ETest:
    ...
    def login_with_credentials(self, username, password):
        ...
        assert response.status_code == 200, f"Login failed: {response.content if hasattr(response, 'content') else 'Unknown error'}"
    ...
    def refresh_token(self, refresh_token):
        ...
        assert response.status_code == 200, f"Token refresh failed: {response.content if hasattr(response, 'content') else 'Unknown error'}"
